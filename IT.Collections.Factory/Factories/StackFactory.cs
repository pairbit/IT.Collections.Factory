...
public class StackFactory : IReadOnlyCollectionFactory
{
    ...
    public Stack<T> Empty<T>(in Comparers<T> comparers = default) => new();

    public Stack<T> New<T>(int capacity, in Comparers<T> comparers = default) => new(capacity);

    public Stack<T> New<T>(int capacity, EnumerableBuilder<T> builder, in Comparers<T> comparers = default)
    {
    ...
    public Stack<T> New<T, TState>(int capacity, EnumerableBuilder<T, TState> builder, in TState state, in Comparers<T> comparers = default)
    {
        if (capacity == 0) return new();
        if (builder == null) throw new ArgumentNullException(nameof(builder));
