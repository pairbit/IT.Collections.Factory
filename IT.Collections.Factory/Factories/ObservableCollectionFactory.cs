...
public class ObservableCollectionFactory : IListFactory, IReadOnlyListFactory
{
    public static readonly ObservableCollectionFactory Default = new();

    public EnumerableType Type => EnumerableType.None;

    public ObservableCollection<T> Empty<T>(in Comparers<T> comparers = default) => new();

    public ObservableCollection<T> New<T>(int capacity, in Comparers<T> comparers = default) => new();

    public ObservableCollection<T> New<T>(int capacity, EnumerableBuilder<T> builder, in Comparers<T> comparers = default)
    {
    ...
    public ObservableCollection<T> New<T, TState>(int capacity, EnumerableBuilder<T, TState> builder, in TState state, in Comparers<T> comparers = default)
    {
        if (capacity == 0) return new();
        if (builder == null) throw new ArgumentNullException(nameof(builder));
